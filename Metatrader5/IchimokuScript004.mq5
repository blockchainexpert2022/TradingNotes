...
void Ichimoku(string sname)
  {
   ...
   if (senkou_span_a_buffer[0] < senkou_span_a_buffer[0]
       && mql_rates[0].open < senkou_span_b_buffer[0]
       && mql_rates[0].close > senkou_span_b_buffer[0]
       ||
       senkou_span_a_buffer[0] > senkou_span_a_buffer[0]
       && mql_rates[0].open < senkou_span_a_buffer[0]
       && mql_rates[0].close > senkou_span_a_buffer[0]
       )
     {
      printf("Signal d'achat détecté pour " + sname + " " + mql_rates[0].close);
     }
   ...
   if (senkou_span_a_buffer[0] < senkou_span_b_buffer[0]
     ...
     {
      printf("Signal de vente détecté pour " + sname + " " + mql_rates[0].close);
     }
